...
See [the design notes](CONSOLE-DESIGN.md) for references, state rules and keyboard behavior. `docs/tour/index.html` has thirteen **illustrative** scenes; it is not a native desktop recording.
...
- The middle group now targets the monitor center on either axis, independent of unequal end groups or disabled modules. A collision clamp shifts it only when needed; too-small screens scroll rather than overlap.
- Optional date is a wallpaper-tinted pixel calendar using the same display face as the time. The approved time digits are unchanged.
- **Workspace manager button** can be hidden independently of workspace gems. Hover the last visible gem for a Yes/Not now prompt; leaving both gem and prompt dismisses it. The manager never opens on hover alone. Library, IPC and the installed keybinding still work.
- **Window previews** uses one live, aspect-fitted, cursor-free window capture for the selected workspace. Hover or keyboard-focus a window row to change the preview. Reduced motion uses a still frame. Captures exist only while the manager is open and the preference enabled, stay in memory, and are not saved or added to history. Missing/unsupported/blocked capture has a fallback; the optional capture component loads by URL so an older Quickshell cannot break shell startup. Native capture permission/protocol behavior remains unverified here.
- The studio thumb has a 20px hit target and explicit grab-offset/content-position mapping. It is outside the thumbnail GridView, with mouse stealing prevented, instead of relying on the old nested attached scrollbar. Track clicks, drag-to-end, keyboard and wheel navigation are supported. The quick strip retains its slider, but shows text only for busy/empty/error states.
- **Advanced…** opens the real NetworkManager connection editor for both Ethernet and Wi-Fi. It now reports missing `nm-connection-editor`/startup failures and releases the shell's exclusive input layer after a successful spawn. The preview does not install `networkmanagerapplet`; it is already declared for the eventual configured environment. No terminal/Starship changes in this polish pass.

For this review, from a fresh extraction:

```sh
bash scripts/preview-shell.sh --software-video --preview-blur
```

`--preview-blur` is an explicit compositor-session operation, not a configuration-file write or Nix activation. Omit it for the original no-rule-change preview. GUI network-editor actions remain real.
...
4. Open panels using the rail, or a second terminal from the same extracted directory:
   ...
   ```sh
   qs ipc --path "$PWD/quickshell/shell.qml" call workspaces toggle
...
- The rail scrolls on short screens; the media marquee becomes a small glyph. Clock/date layout follows the rail orientation. Top-center Settings access stays outside the rail, even with every module hidden.
...
3. Select **Balanced**, **Tinted**, **True black**, or **Paper**. Advanced exposes neutral, tonal, expressive and mono.
4. Adjust **Tone** and **Intensity**. Advanced also exposes representative, dominant or colorful source selection.
...
**Bar:** Left/Right/Top/Bottom position, 35–100% background opacity, and optional blur. **Bar → Placement:** reorder and move each module between top/center/bottom (start/center/end horizontally); clock date is optional. **Bar → Modules:** individually toggle launcher, workspaces, clock, wizard, media, sound/brightness, system, battery/energy, network, Bluetooth, tray, power and settings. Settings and System default off; all others default on; unavailable Bluetooth hardware still hides its control. Workspace indicators: 1–10; rail width: 36–64px. Restore module defaults requires confirmation. Hiding a module does not disable its keyboard shortcut or prevent an already open panel from working.
...
- [ ] Disable unequal end modules and confirm the middle stays centered; check short-screen collision handling. Toggle date and the separate workspace-manager button, then test last-gem hover → Yes/Not now.
- [ ] In Workspaces, preview the selected window, hover/Tab between rows, change workspace and close/reopen. Capture must stop on close; unavailable windows remain usable.
- [ ] Advanced opens a clickable Ethernet/Wi-Fi editor; a missing editor produces a visible error rather than a silent no-op.
- [ ] Test all four rail edges with short/portrait screens and multiple monitors: no overlapping modules, correct reserved work area, corner joins, popup origins, notifications, nested tray menus and bottom OSD.
- [ ] With the new Hyprland rule installed, toggle blur/opacity: only translucent visible surfaces blur, not the transparent desktop or scrim; Settings stays on its current tab and pending backend jobs complete.
- [ ] Open 12+ workspaces; check active overflow gem, manager keyboard navigation, named/special spaces, window focus and exact-target moves (moves refused in preview).
...
- [ ] Right-edge ribbon never opens from hovering; a click opens it on the invoking monitor. Drag the studio scrollbar and quick scrubber through 100+ images; drag release must not apply a wallpaper.
