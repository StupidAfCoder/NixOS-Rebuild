...
Sheet {
    ...
    function move(step) { carousel.currentIndex = Math.max(0, Math.min(carousel.count - 1, carousel.currentIndex + step)); }
    ...
    onShownChanged: if (shown) {
        const index = WallpaperBackend.wallpapers.findIndex(w => w.path === (Settings.previewMode ? WallpaperBackend.previewPath : WallpaperBackend.currentPath));
        carousel.currentIndex = Math.max(0, index);
    }
    ListView {
        id: carousel
    ...
    RowLayout {
        ...
        PixelButton { text: "Adjust colors"; onClicked: { QuickWallpapers.hide(); WallpaperLauncher.toggle(); } }
    }
